...
old_text = """- **Known remaining demo-scope gaps**: no auth on general API routes, no per-user data isolation, no integration/E2E tests, and the notify endpoint's cron secret is skipped when unset (demo).
...
new_text = """- **Known remaining demo-scope gaps**: no auth on general API routes, no per-user data isolation, no integration/E2E tests, and the notify endpoint's cron secret is skipped when unset (demo).
...
if old_text in content:
    ...
    if idx >= 0:
        ...
        print("Looking for: Known remaining demo-scope gaps")
        idx = content.find("Known remaining demo-scope gaps")
        if idx >= 0:
            print(f"Found 'Known remaining' at {idx}")
